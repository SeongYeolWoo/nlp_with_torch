...
class MyEngine(Engine):
    def __init__(self, func, model, crit, optimzier, config):
        self.model = model
        self.crit = crit
    ...
    def train(engine, mini_batch):
        ...
        # 학습될수록 커짐(모델이 복잡해지면서 커짐.)
        p_norm = float(get_parameter_norm(engine.model.parameters()))
        # 학습될수록 작아짐(가면갈수록 기울기가 줄어듬)
    ...
    def attach(train_engine, validation_engine, verbose = VERBOSE_BATCH_WISE):
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_train_logs(engine):
                print('Epoch {} - |param|={:.2e} |g_param|={:.2e} loss={:.4e} accuracy={:.4f}'.format(engine.state.epoch, engine.state.metrics['|param|'], engine.state.metrics['|g_param|'], engine.state.metrics['loss'], engine.state.metrics['accuracy']))
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_train_logs(engine):
                print('Validation - loss={:.4e} accuracy={:.4f} best_loss={:.4e}'.format(engine.state.metrics['loss'], engine.state.metrics['accuracy'], engine.best_loss))
